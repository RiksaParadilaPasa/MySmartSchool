...
public class Adapter_Murid_Admin extends RecyclerView.Adapter<Adapter_Murid_Admin.ViewHolder> {

    String name;


    private List<Entity_Murid_Admin> myteacherList;
    private Context mContext;



    public Adapter_Murid_Admin(Context mContext, List<Entity_Murid_Admin> myteacherList){
...
//
    ...
    @Override
    public Adapter_Murid_Admin.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        //ini memanggil list item REcycleerView(XML) dan ditampung pada variabel v
        View v = LayoutInflater.from(parent.getContext()).inflate(
                R.layout.recycler_item_murid,
                parent,
                false);

        //ini untuk menset VIewHolder dengan item list di atas
        Adapter_Murid_Admin.ViewHolder vh = new Adapter_Murid_Admin.ViewHolder(v);
    ...
    @Override
    public void onBindViewHolder(Adapter_Murid_Admin.ViewHolder holder, final int position) {
//          ini untuk inisialisasi entity
        Entity_Murid_Admin myteacher = myteacherList.get(position);

//             ini untuk mengambil data dari ENtity dan nanti akan diambil dan di set/tampilkan oleh MainACtivity(RecyclerActivity)
        holder.nameGuru.setText(String.valueOf(myteacher.getNama_murid()));
...
//                    ini ngirim data untuk di ambil pada activity lain
                bundle.putString("nis",myteacherList.get(position).getNis_murid());
                bundle.putString("nama",myteacherList.get(position).getNama_murid());
                bundle.putString("nomorhp",myteacherList.get(position).getNomorHp_murid());
                bundle.putString("password",myteacherList.get(position).getPassword());
                //                    ini pindah activity
