...
public class Adapter_Kelas_Admin extends RecyclerView.Adapter<Adapter_Kelas_Admin.ViewHolder> {
    ...
    public Adapter_Kelas_Admin(Context mContext, List<Entity_Kelas_Admin> myteacherList){
...
//
    ...
    @Override
    public Adapter_Kelas_Admin.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        //ini memanggil list item REcycleerView(XML) dan ditampung pada variabel v
        View v = LayoutInflater.from(parent.getContext()).inflate(
                R.layout.recycler_item_kelas,
                parent,
                false);

        //ini untuk menset VIewHolder dengan item list di atas
        Adapter_Kelas_Admin.ViewHolder vh = new Adapter_Kelas_Admin.ViewHolder(v);
    ...
    @Override
    public void onBindViewHolder(Adapter_Kelas_Admin.ViewHolder holder, final int position) {
//          ini untuk inisialisasi entity
        Entity_Kelas_Admin myteacher = myteacherList.get(position);

//             ini untuk mengambil data dari ENtity dan nanti akan diambil dan di set/tampilkan oleh MainACtivity(RecyclerActivity)
        holder.nameGuru.setText(String.valueOf(myteacher.getNamaKelas()));
...
//                    ini ngirim data untuk di ambil pada activity lain
                bundle.putString("id",myteacherList.get(position).getIdKelas());
                bundle.putString("nama",myteacherList.get(position).getNamaKelas());
                //                    ini pindah activity
